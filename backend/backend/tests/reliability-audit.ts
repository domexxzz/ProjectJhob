...
async function testAiHonesty(token: string): Promise<void> {
  ...
  //
  // ⚠️ ข้อนี้เคยแจ้งเตือนผิด 2 ครั้ง เพราะไปจับรูปแบบข้อความแทนที่จะดูความหมาย
  // ครั้งล่าสุด AI ตอบถูกต้องว่า "ยังไม่มีการบันทึกผ่อนรถเลย" แล้วแนะนำต่อ
  // แต่ regex ไปเจอตัวเลขในคำแนะนำเลยตัดว่ากุข้อมูล
  //
  // ตรรกะใหม่: ถ้า AI ยอมรับตรง ๆ ว่าไม่มีข้อมูล = ซื่อสัตย์แล้ว จบ
  ...
  check(
    ...
    fabricated
      ...
      : admitsNoData
        ...
        : 'ไม่ได้ระบุยอดค่าผ่อนรถขึ้นมาเอง',
  );
...
async function testAvailability(token: string): Promise<void> {
  ...
  // ระบบส่งอีเมลใช้กับ OTP (ลืมรหัสผ่าน/ยืนยันอีเมล/ล็อกอิน) ถ้าหลุดไปผู้ใช้จะ
  // เข้าระบบไม่ได้เลยและไม่มีใครรู้จนกว่าจะมีคนมาบ่น
  // ตรวจแค่ว่า "ตั้งค่าไว้ไหม" ไม่ส่งอีเมลจริงทุกรอบ เพราะบัญชีทดสอบใช้โดเมน
  // .invalid การส่งแล้วตีกลับซ้ำ ๆ ทุกวันจะทำให้ผู้ให้บริการลดความน่าเชื่อถือ
  // ของผู้ส่ง จนอีเมลจริงตกสแปมมากขึ้น
  check(
    'ระบบส่งอีเมลพร้อมใช้งาน (ใช้กับ OTP)',
    health?.mailer === 'ok',
