...
export async function fetchPrediction(userId: string, timeoutMs = 20000): Promise<PredictionResult | null> {
  const [user, rawTxns] = await Promise.all([
  ...
  const transactions = rawTxns.reverse();
