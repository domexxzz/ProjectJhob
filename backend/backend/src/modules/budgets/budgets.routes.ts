...
budgetsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [categorySums, budgetSums] = await Promise.all([
    ...
    for (const row of categorySums) {
      if (row.categoryId) {
        categoryMap.set(row.categoryId, row._sum.amount || 0);
      }
    }
    ...
    for (const row of budgetSums) {
      if (row.budgetId) {
        budgetMap.set(row.budgetId, row._sum.amount || 0);
      }
    }

    const budgetsStatus = budgets.map((b) => {
      const spent = b.categoryId
        ? (categoryMap.get(b.categoryId) || 0)
        : (budgetMap.get(b.id) || 0);
