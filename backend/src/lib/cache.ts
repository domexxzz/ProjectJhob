...
class CacheManager {
  ...
  constructor() {
    ...
    if (redisUrl) {
      ...
      this.useMemory = false; // Cold start: avoid bypassing Redis while connection is establishing
      try {
  ...
  async delPattern(pattern: string): Promise<void> {
    ...
    if (!this.useMemory && this.redis) {
      try {
        ...
        console.warn(`[Cache] Redis scan/del pattern error: ${e.message}. Invalidating memory cache.`);
      }
    }
