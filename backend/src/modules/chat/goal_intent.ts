...
const GOAL_VERB =
  /ตั้งเป้า|เป้าหมาย|สร้างเป้าหมาย|สร้างเป้า|อยากเก็บเงิน|อยากออม|อยากเก็บตัง|วางแผนเก็บเงิน|วางแผนออม|เก็บเงินซื้อ|เก็บตังซื้อ|ออมเงินซื้อ|เก็บเงินไป|เก็บเงินเพื่อ|เป้าออม/;

/** คำถาม — ผู้ใช้ถามข้อมูล ไม่ได้สั่งให้สร้าง (ยกเว้นสั่งสร้างเป้าหมายชัดเจน) */
...
/**
 * ดึงชื่อเป้าหมายจากข้อความ เช่น "สร้างเป้าหมายเทรด" → "เทรด", "เก็บเงินซื้อโน้ตบุ๊ค 30000" → "ซื้อโน้ตบุ๊ค"
 */
function extractName(message: string): string {
  const patterns = [
    /(?:สร้างเป้าหมาย|ตั้งเป้าหมาย|ตั้งเป้า|เป้าหมายหัวข้อ|เป้าหมาย|สร้างเป้า)\s*([^\d\s]{1,30})/,
  ...
  for (const re of patterns) {
    ...
    if (m) {
      const raw = m[1]
        .replace(/(?:บาท|฿|ภายใน|ให้หน่อย|หน่อย|ครับ|ค่ะ|นะ|จ้า|การออม|หัวข้อ)/g, '')
        .replace(/\s{2,}/g, ' ')
        .trim();
      if (raw.length >= 1) return raw.slice(0, 80);
    }
  }
  return 'เป้าหมายออม';
}
...
function extractTargetBaht(message: string): number | null {
  ...
  return max <= 20_000_000 ? max : null;
}

/**
 * คืน GoalIntent ถ้ามั่นใจว่าผู้ใช้สั่งให้สร้างเป้าหมายออม
 */
export function detectGoalIntent(message: string): GoalIntent | null {
  const m = message.trim();
  if (!GOAL_VERB.test(m)) return null;

  const isExplicitCreate = /สร้างเป้าหมาย|ตั้งเป้า|สร้างเป้า|เป้าหมายหัวข้อ|ให้สร้างเป้า/i.test(m);
  if (!isExplicitCreate && IS_QUESTION.test(m)) return null;
  if (IS_ADD_TO_GOAL.test(m)) return null;
  if (ALREADY_PAID.test(m)) return null;
  if (WANTS_FILE.test(m)) return null;

  let targetBaht = extractTargetBaht(m);
  if (targetBaht === null && isExplicitCreate) {
    targetBaht = 10000; // ค่าเริ่มต้นถ้าไม่ได้ระบุยอดเงินเป้าหมาย
  }
