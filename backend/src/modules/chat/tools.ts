...
export const TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'create_budget',
      description:
        'ตั้งงบประมาณใหม่ให้ผู้ใช้ ใช้เมื่อผู้ใช้บอกว่าอยากตั้งงบประมาณ เช่น "ตั้งงบอาหาร 5000", "ตั้งงบประมาณ 10000", "ตั้งงบช็อปปิ้ง 3000". หลังตั้งงบให้ยืนยันสั้น ๆ',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string', description: 'ชื่อหมวดหรือชื่อหัวข้องบ เช่น "อาหาร", "ช็อปปิ้ง", "งบรวม"' },
          amountBaht: { type: 'number', description: 'จำนวนเงินงบประมาณ หน่วยบาท (ต้องมากกว่า 0)' },
          period: { type: 'string', enum: ['monthly', 'weekly'], description: 'ระยะเวลา: "monthly" (ต่อเดือน) หรือ "weekly" (ต่อสัปดาห์)' },
        },
        required: ['amountBaht'],
      },
    },
...
export const INCOME_HINT =
  /เงินเข้า|เงินโอนเข้า|โอนเข้า|เข้าบัญชี|รับโอน|ได้รับเงิน|เงินเดือน|โบนัส|ค่าจ้าง|ฟรีแลนซ์|freelance|พาร์ทไทม์|part.?time|รับเงิน|ได้เงิน|เงินได้|รายได้|เงินคืน|ขายของ|ขายได้|ดอกเบี้ย|ปันผล|ค่าขนม|เงินทอน|refund|cashback|เทรด|กำไร/i;
...
/**
 * ตรวจ "การจดแบบสั้น" สไตล์ป้านวล เช่น "ก๋วยเตี๋ยว 55", "เงินเดือน 30000", "ได้เงินจากเทรด 10 ดอลล่า"
 */
export function detectQuickLog(
  ...
  // 1) ลองจับกรณีระบุสกุลเงินต่างประเทศ เช่น "10 ดอลล่า", "10 usd", "10$"
  const usdMatch = m.match(/^(.*?)\s*(\d[\d,]*(?:\.\d{1,2})?)\s*(?:ดอลลาร์|ดอลล่า|ดอลลา|usd|\$)\s*(.*?)$/i);
  if (usdMatch) {
    const amountUsd = Number(usdMatch[2].replace(/,/g, ''));
    if (Number.isFinite(amountUsd) && amountUsd > 0) {
      const amountBaht = Math.round(amountUsd * 34); // แปลงเป็น THB (อัตราประมาณ 34 บาท/USD)
      const prefix = usdMatch[1].trim();
      const suffix = usdMatch[3].trim();
      let rawNote = `${prefix} ${suffix}`.trim();
      rawNote = rawNote.replace(/^(จ่าย|ซื้อ|ชำระ|เพิ่ม|ได้|ได้รับ)\s*/i, '').replace(/\s*(เข้ารายได้|เข้าเป้าหมาย|เข้ารายจ่าย)$/i, '').trim();
      const type = INCOME_HINT.test(m) ? 'income' : 'expense';
      const cleanNote = rawNote ? `${rawNote} (${amountUsd} USD = ${amountBaht.toLocaleString('en-US')} บาท)` : `${type === 'income' ? 'รายรับ' : 'รายจ่าย'} (${amountUsd} USD = ${amountBaht.toLocaleString('en-US')} บาท)`;
      return { type, amountBaht, note: cleanNote };
    }
  }

  // 2) จับรูปแบบปกติ "<ชื่อ> <จำนวน> [บาท/฿]"
...
/**
 * ตรวจจับเจตนา "ตั้งงบประมาณ" จากข้อความแชต เช่น "ตั้งงบอาหาร 5000", "ตั้งงบช็อปปิ้ง 3000"
 */
export function detectBudgetIntent(message: string): BudgetIntent | null {
  const m = message.trim();
  if (!/(?:ตั้งงบ|สร้างงบ|งบประมาณ|ตั้งงบประมาณ|งบเดือน|งบสัปดาห์)/i.test(m)) return null;
  if (/[?？]|เท่าไหร่|เท่าไร|กี่บาท|คืออะไร/i.test(m)) return null;

  const numMatch = m.match(/(\d[\d,]*(?:\.\d{1,2})?)/);
  if (!numMatch) return null;
  const amountBaht = Number(numMatch[1].replace(/,/g, ''));
  if (!Number.isFinite(amountBaht) || amountBaht <= 0) return null;

  let name = m
    .replace(/(?:ตั้งงบประมาณ|ตั้งงบ|สร้างงบ|งบประมาณ|งบเดือน|งบสัปดาห์|ต่อเดือน|ต่อสัปดาห์|บาท|฿|ให้หน่อย|หน่อย|ครับ|ค่ะ)/gi, '')
    .replace(numMatch[1], '')
    .replace(/\s{2,}/g, ' ')
    .trim();

  const period = /สัปดาห์|อาทิตย์|weekly/i.test(m) ? 'weekly' : 'monthly';
  return { name: name || 'งบรวม', amountBaht, period };
}

/** สร้างงบประมาณใหม่ให้ผู้ใช้ */
export async function createBudgetTool(userId: string, args: any): Promise<ToolResult> {
  const amountBaht = Number(args.amountBaht);
  if (!Number.isFinite(amountBaht) || amountBaht <= 0) {
    return { error: 'amountBaht ต้องเป็นตัวเลขมากกว่า 0' };
  }
  ...
  let categoryId: string | null = null;
  let name: string | null = rawName || null;

  if (rawName && rawName !== 'งบรวม') {
    const cat = await resolveCategory(rawName, 'expense');
    if (cat) {
      categoryId = cat.id;
      name = cat.nameTh;
    }
  ...
  let existing = null;
  if (categoryId) {
    existing = await prisma.budget.findFirst({ where: { userId, categoryId, period } });
  } else if (name) {
    existing = await prisma.budget.findFirst({ where: { userId, name, period } });
  }
  ...
  if (existing) {
    ...
    budget = await prisma.budget.create({
      data: { userId, name: name ?? 'งบรวม', categoryId, amount, period },
      include: { category: true },
    });
  }
  ...
  return {
    ok: true,
    id: budget.id,
    name: budget.category?.nameTh ?? budget.name ?? 'งบรวม',
    amountBaht: satangToBaht(budget.amount),
...
export async function runTool(name: string, args: any, userId: string): Promise<ToolResult> {
  try {
    switch (name) {
      ...
      case 'create_budget':
        return await createBudgetTool(userId, args ?? {});
      default:
