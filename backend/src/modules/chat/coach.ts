...
- ⚠️ แยกให้ชัด "เป้าหมาย" กับ "รายจ่าย":
  · "อยากเก็บเงินซื้อโน้ตบุ๊ค 30000", "ตั้งเป้าออม 50000" = ยังไม่ได้จ่าย → เรียก create_goal (ห้ามใช้ create_transaction เด็ดขาด)
  · "จ่ายค่าโน้ตบุ๊ค 30000" = จ่ายไปแล้ว → create_transaction
  · "หยอดกระปุกเป้าโน้ตบุ๊ค 500" = เก็บเข้าเป้า → add_to_goal
- 🚫 ห้ามบอกว่า "บันทึกให้แล้ว/จดให้แล้ว/สร้างเป้าหมายให้แล้ว" ถ้ายังไม่ได้เรียกเครื่องมือจริงและได้ผลลัพธ์ ok กลับมา
  ถ้าเครื่องมือคืน error ให้บอกผู้ใช้ตามตรงว่าทำไม่สำเร็จ อย่าแกล้งทำเป็นว่าสำเร็จ
  ...
  for (const p of providers) {
    ...
    try {
      ...
      for (let step = 0; step < MAX_STEPS; step++) {
        ...
        timer?.add(`llm:${p.name}#${step + 1}`, Date.now() - llmStartedAt);
        ...
        if (toolCalls && toolCalls.length) {
          ...
          for (const tc of toolCalls) {
            ...
            timer?.add(`tool:${tc.function.name}`, Date.now() - toolStartedAt);
      ...
      // นับเวลาที่เสียไปกับ provider ที่ล้ม — ถ้าตัวแรกตาย ทุก request จะจ่ายค่านี้ฟรี ๆ
      timer?.add(`llmfail:${p.name}`, Date.now() - providerStartedAt);
...
/** OCR รูป (สลิป/เอกสาร) รองรับ Typhoon OCR, OpenAI Vision และ Groq Vision */
export async function ocrImage(imageDataUrl: string): Promise<string> {
  const promptText = 'อ่านข้อความทั้งหมดในรูปสลิปหรือเอกสารนี้ออกมาเป็นข้อความภาษาไทยล้วน เน้น จำนวนเงิน วันที่และเวลา ชื่อร้านค้าหรือผู้รับเงิน และเลขอ้างอิงรายการ';

  // 1. Typhoon OCR (แนะนำสำหรับสลิปไทย)
  if (env.typhoonApiKey) {
    try {
      ...
      if (result) {
        // ป้องกัน hallucination จากเลขบัญชีที่ถูกเซ็นเซอร์ (***) กลายเป็น * ซ้ำๆ ไม่สิ้นสุด
        result = result.replace(/(?:\*\s*){5,}/g, '***');
        return result;
      }
    } catch (e) {
      console.warn('[OCR:Typhoon] ล้มเหลว:', (e as Error).message);
    }
  }

  // 2. OpenAI Vision (gpt-4o / gpt-4o-mini)
  if (env.openaiApiKey) {
    try {
      ...
      console.warn('[OCR:OpenAI] ล้มเหลว:', (e as Error).message);
    }
  }
  ...
  if (env.groqApiKey) {
    try {
      const client = new OpenAI({ apiKey: env.groqApiKey, baseURL: 'https://api.groq.com/openai/v1' });
      const resp = await client.chat.completions.create({
      ...
      console.warn('[OCR:Groq] ล้มเหลว:', (e as Error).message);
    }
  }

  throw new Error('ยังไม่ได้ตั้งค่า TYPHOON_API_KEY, OPENAI_API_KEY หรือ GROQ_API_KEY สำหรับอ่านสลิปใน backend/.env');
}
