...
export function parseAmount(text: string): number | null {
  ...
  const paid = text.match(
    new RegExp(
      `(?:จำนวนเงินที่ชำระ|ยอดที่ต้องชำระ|ยอดที่ชำระ|ยอดชำระ|รวมชำระ|จำนวนที่ชำระ|ยอดสุทธิ)${SEP}(\\d[\\d,]*(?:\\.\\d{1,2})?)`,
    ),
  );
  if (paid) return toSatang(paid[1]);

  // 2) ป้าย "จำนวนเงิน" ทั่วไป (จำนวนเต็มหรือทศนิยม)
  const labeled = text.match(new RegExp(`จำนวน(?:เงิน)?${SEP}(\\d[\\d,]*(?:\\.\\d{1,2})?)`));
  ...
  for (const m of text.matchAll(/(\d[\d,]*(?:\.\d{1,2})?)\s*(?:บาท|฿)/g)) {
...
export function parseMerchant(text: string): string | null {
  const patterns = [
    ...
    // K+ slip format with arrow: ธ.กสิกรไทย, xxx-x-x2535-x, ↓, ชิดเก้นซีสไก่เขย่า, นาง กรรณิการ์...
    /(?:↓|->|>>)\s*(?:,\s*)?([^,]+)/,
  ];
