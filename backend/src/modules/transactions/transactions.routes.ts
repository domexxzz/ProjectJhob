...
transactionsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      console.warn('[parse-slip] OCR warning (falling back gracefully):', (e as Error).message);
    }

    const amountSatang = text ? parseAmount(text) : 0;
    const date = text ? parseDate(text) : null;
    const ref = text ? parseRef(text) : null;
    const merchant = text ? parseMerchant(text) : null;
    const categoryId = text ? await autoCategorize(text, merchant || '', 'expense', req.userId) : null;

    res.json({
      amount: amountSatang ?? 0, // สตางค์ (0 = อ่านยอดไม่เจอ ให้ผู้ใช้กรอกเอง)
      date: date ? date.toISOString().split('T')[0] : null,
      ref,
      merchant,
      categoryId,
      rawText: text,
      message: text ? undefined : 'ไม่สามารถอ่านข้อความจากสลิปด้วย OCR ได้ กรุณากรอกข้อมูลด้วยตนเอง',
    });
  }),
);
