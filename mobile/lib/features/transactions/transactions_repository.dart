...
class TransactionsRepository {
  ...
  bool _isNetworkOrTimeoutError(Object e) {
    if (e is DioException) {
      final status = e.response?.statusCode;
      if (status != null && status >= 400 && status < 500) {
        return false; // 4xx validation/client error -> do not queue, rethrow!
      }
      return true; // Network, timeout, server error or connection error -> queue offline
    }
    return false;
  }
  ...
  Future<String?> create({
    ...
    try {
      ...
      if (!_isNetworkOrTimeoutError(e)) rethrow;
