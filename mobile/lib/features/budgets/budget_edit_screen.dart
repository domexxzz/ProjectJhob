...
class BudgetEditScreen extends ConsumerStatefulWidget {
  const BudgetEditScreen({super.key, this.status});
...
class _BudgetEditScreenState extends ConsumerState<BudgetEditScreen> {
  ...
  void initState() {
    super.initState();
    _showOnDashboard = widget.status?.showOnDashboard ?? true;
  ...
  Widget build(BuildContext context) {
    ...
    if (!_didApplyDisplayCurrency) {
      _amountController.text = Money.format(widget.status?.amount ?? 0);
    ...
    final pct = targetAmountInSubunits > 0
        ? ((widget.status?.spent ?? 0) / targetAmountInSubunits).clamp(0.0, 9.9)
        : 0.0;
    ...
    final remaining = (targetAmountInSubunits - (widget.status?.spent ?? 0))
                      ...
                      child: Column(
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              Text(
                                Money.formatBaht(widget.status?.spent ?? 0),
                                style: TextStyle(
                                    color: color,
                                    fontWeight: FontWeight.bold,
                                    fontSize: 24),
  ...
  Future<void> _saveBudget() async {
    final statusId = widget.status?.id;
  ...
  Future<void> _deleteBudget() async {
    final statusId = widget.status?.id;
    if (statusId == null) return;
    await ref.read(transactionsRepoProvider).deleteBudget(statusId);
