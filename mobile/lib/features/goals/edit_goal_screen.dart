...
class _EditGoalScreenState extends ConsumerState<EditGoalScreen> {
  ...
  void initState() {
    ...
    if (widget.goalId != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final goal = ref.read(goalsProvider).firstWhereOrNull((g) => g.id == widget.goalId);
        if (goal == null) return;
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final int currentSavings = widget.goalId != null
        ? (goals.firstWhereOrNull((g) => g.id == widget.goalId)?.current ?? 0)
        : 0;
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  image: DecorationImage(
                                    image: _getImageProvider(_selectedImageFile!.path),
                                    fit: BoxFit.cover,
                                  ),
                                  border: Border.all(color: const Color(0xFF4CD97B), width: 3),
                                  boxShadow: [
